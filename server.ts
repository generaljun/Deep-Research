...
const getLevelPrompt = (length: string) => {
  if (length === 'collection') {
    return "【任务定位：纯数据提取与表格整理】\n本任务必须抛弃所有“报告”写法。不需要任何前言、总结、背景分析。你的全部任务就是：把找来的参考资料一条条提炼出来，塞进 Markdown 表格里。表格行数越多越好，尽可能详尽地榨干资料。";
  }
...
const getCriticLevelPrompt = (length: string) => {
  if (length === 'collection') {
    return `5. 格式强制（纯表格）：如果草稿中出现了除了表格和必要的参考文献来源之外的大段文字、分析总结、水文，必须不留情面地打回！这是一份纯数据收集表格。如果发现草稿没有尽最大可能将信息塞进表格里，请打回并要求重写。`;
  }
  if (length.includes('3000')) {
  ...
  "chapters": [
    ...
    if (currentChapterIndex === 0 && chapterStates.length === 0) {
      ...
      try {
        ...
        `你是一个单纯的格式整理员。根据核心要点，输出一句类似于“以下是为您收集到的关于【${outline.report_title}】的相关信息数据表：”的话即可，不要其他任何废话。` :
...
请严格按照以下 JSON 格式输出（不要输出任何其他内容）：
{
  ...
  "new_queries": ["2025年 Q1 固态电池 市场规模", "2025年 宁德时代 固态电池 进展"]
      ...
      try {
        ...
        `你是一个冷血无情的数据清洗提取机器。
当前系统日期：${currentDateStr}。
请将提供的【参考素材】中所有的事实、新闻、数据、案例全部提取出来，整理成一个无边无际的巨大 Markdown 表格。
${levelPrompt}

【纯粹收集规范】
1. 必须纯表格：除了必要的二级标题（## 第 ${i + 1} 章：${cleanTitle}）之外，正文内容必须100%是 Markdown 表格！不要有任何段落解释、前导语或总结陈词。如果写了一段段的文字，将被视为严重失败。
2. 表格设计：表格必须包含至少以下列：【时间】、【核心实体（企业/机构名）】、【事件/数据详情】、【资料来源】。你可以根据课题自主调整或增加合适的列名。
3. 贪婪提取，越全越好：不要放过素材中的任何一个有效案例、数字和事件，一行一行地添加到表格里，表格行数越多越佳。
4. 严格找准出处：【资料来源】列中必须明确标明该条数据的来源序号（如 [1], [2]），与文末的参考文献序号一一对应。
5. 必须要有标准的参考文献：在表格最下方，必须专门设立一个“### 参考文献与数据源”小节。
...
${searchResults}`;
        ...
        while (retryCount <= maxRetries) {
          ...
          `你是一位严苛的无情查表员。当前系统日期：${currentDateStr}。请严格检查【生成的表格结果】。

【查表标准】
1. 纯表格属性（致命项）：结果是否大篇幅写了非表格的段落文字？除了最开头的二级标题和最后的参考文献，如果中间有任何“报告总结”、“分析解释”、“过渡段落”等纯文字，必须打回！
2. 数量与遗漏（致命项）：是否穷尽提取了【参考素材】中的相关信息？表里的条目越丰富越好。如果素材里有大量案例但最终只输出了一两行，必须严厉打回。
3. 出处核查与参考文献（致命项）：表格中【资料来源】列是否准确标注了来源引用序号（如 [1]）？而且最下方是否附带了严格遵循学术规范（比如类似 GB/T 7714 排版）的“### 参考文献与数据源”清单？如果两者缺失其一或格式不标准，打回！
4. 有无水文（致命项）：是不是混杂了行业大背景、发展意义等垃圾信息？如果有，立即打回。
${criticLevelPrompt}
